...
with tf.Session() as sess:
    ...
    saver.save(sess, "./Model/SpaceInvaders_DemonAttack_CNN" + "/model" + str("_global_step") + ".cptk")
    end = datetime.now()
    print(f"Training time:{end - start}")
    df = pd.DataFrame.from_dict({"Rewards": returns_list,"Global_Step":step_list})
    df.to_excel("./Excel/Hybrid_SpaceInvaders_CNN.xlsx", index=False)
    df = pd.DataFrame.from_dict({"Rewards": returns_list2,"Global_Step":step_list2})
    df.to_excel("./Excel/Hybrid_DemonAttack_CNN.xlsx", index=False)
